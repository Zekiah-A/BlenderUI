...
{
	glColor3f(1, 1, 1);
	Rect *r1 = new Rect(0 + locx, locy, 0.1, 0.05, 1);
	glColor3f(0.5, 0.5, 0.5);
	Rect *r2 = new Rect(0 + locx, locy, 0.09, 0.04, 1);
	glColor3f(0.7, 0.7, 0.7);
	Rect *r3 = new Rect(-0.025 + locx, locy, 0.04, 0.04, 1);
}

void Main::draw_checkbox(float locx, float locy, bool ischecked)
{
	Rect *r1 = new Rect(locx, locy, 0.06, 0.06, 1);

	if (ischecked == true)
		glColor3f(0, 1, 0);
	else
		glColor3f(1, 0, 0);
	Rect *r2 = new Rect(locx, locy, 0.05, 0.05, 1);
	glColor3f(1, 1, 1);
}

void Main::draw_textbox(float locx, float locy)
{       //outer design rect
	Rect *r1 = new Rect(locx, locy, 0.2, 0.05, 1);
	//inner text entry box
	glColor3f(0.7, 0.7, 0.7);
	Rect *r2 = new Rect(locx, locy, 0.19, 0.04, 1);	
}
